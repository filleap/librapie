  ...
  <body>
 	 <div class="container">
		...
		?>
			    ...
			    <caption>Liste des livres commandés et à prendre pour l'année <?php print($annee); ?></caption>
					...
					<?php
						...
						for ($i=0; $i < sizeof($listeLivres); $i++) {
							...
							$query = "SELECT Qte FROM ".$annee."_APIE_Commandes WHERE EAN=$ean AND Donne='0'";
							$result = $cnx->query($query) or die($cnx->error);
							while ($row = mysqli_fetch_object($result)) {
							...
							// soustraction des livres en stock
							...
							if ($qteCdee > 0) {
								$qteCdee = $qteCdee - $listeStock[0]->Qte;
							}
			...
			<h2 size="10px"><?php print("Nombre de livres à prendre: ".$nbLivresAprendre); ?></h2>
		</div>
	</body>
</html>
